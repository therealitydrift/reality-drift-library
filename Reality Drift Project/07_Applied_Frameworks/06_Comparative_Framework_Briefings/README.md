# Reality Drift: Comparative Framework Briefings

README v2

This collection contains comparative briefing documents designed to distinguish related concepts that are often conflated in discussions about AI systems, digital environments, optimization, and institutional behavior.

Each document contrasts two adjacent concepts while explaining the deeper structural pattern underneath them through the Reality Drift framework.

---

## Comparative Framework Briefings

- [AI Slop vs. Synthetic Realness](./01_AI_Slop_vs_Synthetic_Realness.pdf)  
  Distinguishes obvious low-quality AI output from polished synthetic content that feels emotionally believable without being grounded.

- [Optimization Trap vs. Goodhart’s Law](./02_Optimization_Trap_vs_Goodharts_Law.pdf)  
  Explains the difference between metric distortion and broader systems reorganizing around measurable proxies.

- [Filter Fatigue vs. Information Overload](./03_Filter_Fatigue_vs_Information_Overload.pdf)  
  Separates the problem of excessive information from the exhaustion of constant credibility checking and interpretation.

---

## Framework Context

These comparative briefings describe recurring patterns across AI systems, media environments, institutions, and digital life:

- Systems appearing optimized while weakening their relationship to reality  
- Polished outputs replacing grounded experience  
- Metrics improving while meaning declines  
- Information environments becoming cognitively exhausting to navigate  
- Trust shifting from reality to surface-level coherence and fluency  

Reality Drift describes this broader condition. Systems continue functioning while gradually losing alignment with the reality they were originally built to represent, measure, or respond to.

---

## Related Concepts

- Reality Drift  
- Semantic Fidelity  
- Synthetic Realness  
- Optimization Trap  
- Filter Fatigue  
- Constraint Collapse  
- Proxy Optimization  
- Narrative Drift  

---

## Additional Resources

- [Substack (articles)](https://therealitydrift.substack.com/)
- [GitHub (full library)](https://github.com/therealitydrift/reality-drift-library)
- [DOI (research paper)](https://dx.doi.org/10.2139/ssrn.6150706)
- [Glossary & Definition](https://offbrandguy.com/reality-drift-glossary/)
